...
using namespace std;
//rec solution
int solve(vector<int>& rods,int s1,int s2,int i,int n){
    if(i>=n){
        if(s1==s2){
            return s1;
        }
        return 0;
                ...
                //not taking any            taking in s1                    taking in s2
    return max(solve(rods,s1,s2,i+1,n),max(solve(rods,s1+rods[i],s2,i+1,n),solve(rods,s1,s2+rods[i],i+1,n)));
}
//dp solution
